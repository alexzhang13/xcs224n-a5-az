...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sentence in sents:
            ...
            for word in sentence:
                word_list = [self.start_of_word]
                for char in word:
                    word_list.append(self.char2id[char])
                word_list.append(self.end_of_word)
                sentence_list.append(word_list)
            c_sents.append(sentence_list)
        return c_sents
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        char_sents_padded = pad_sents_char(char_sents, self.char2id['<pad>'])
        sents_var = torch.tensor(char_sents_padded, dtype=torch.long, device=device)
        return torch.transpose(sents_var, 0, 1).contiguous()
